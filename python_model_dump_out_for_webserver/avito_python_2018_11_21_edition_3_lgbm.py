...
import pickle
with open('CountVectorizer.pickle', 'rb') as handle2:
    CountVectorizer = pickle.load(handle2)
    
with open('TfidfVectorizer.pickle', 'rb') as handle3:
    TfidfVectorizer = pickle.load(handle3)
    
title_counts = CountVectorizer.transform(data['title'])
desc_counts = TfidfVectorizer.transform(data['description'])

###
predictors = [
    'price', 'image_confidence'
]
categorical = [
...
#data['parent_category_name'] = data['parent_category_name'].astype('float64')
#data['category_name '] = data['category_name'].astype('float64')
...
lgb_clf_predict = lgb.Booster(model_file='lgbm_ALLmodel2.txt')
res = lgb_clf_predict.predict(x_test)[0]*100
